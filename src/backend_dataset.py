...
class RAVENDataset(Dataset):
    ...
    def __init__(self, dataset_type,data_dir, constellation_filter,rule_filter = "", attribute_filter = "",
        n_train = None):
    ...
    def filter_dataset(self, attribute_filter, rule_filter):
        ...
        if attribute_filter == "" or rule_filter == "":
            filtered_indeces = list(range(num_indeces))
        else:
            for index in range(num_indeces):
                ...
                if self.dataset_type == "train" or self.dataset_type == "val":
                    if rules[attribute_idx[attribute_filter]] not in self.rule_idx(rule_filter, attribute_filter):
                        filtered_indeces.append(index)
                else:
                    if rules[attribute_idx[attribute_filter]] in self.rule_idx(rule_filter, attribute_filter):
                        filtered_indeces.append(index)
